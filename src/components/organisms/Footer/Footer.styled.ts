import styled from "styled-components";
import footerBackground from "../../../assets/images/Footer_Background.svg";

export const FooterStyled = styled.footer`
  min-height: 598px;
  box-sizing: border-box;
  padding: 64px 0 42px;
  color: #ffffff;
  background-color: #076f9f;
  background-image: url(${footerBackground});
  background-position: center;
  background-size: cover;
`;

export const FooterContentStyled = styled.div`
  width: min(87.6%, 1262px);
  margin: 0 auto;
  display: grid;
  grid-template-columns: minmax(0, 600px) minmax(0, 1fr);
  gap: 60px;

  @media (max-width: 900px) {
    grid-template-columns: 1fr;
  }
`;

export const FooterColumnStyled = styled.section`
  p,
  h3 {
    color: inherit;
  }

  h3 {
    margin: 0 0 22px;
    font-family: ${({ theme }) => theme.fonts.body};
    font-size: 16px;
    line-height: 1.2;
    text-transform: uppercase;
  }

  p {
    margin: 0 0 22px;
    font-size: 16px;
    line-height: 1.35;
  }
`;

export const NewsletterFormStyled = styled.form`
  display: grid;
  grid-template-columns: minmax(0, 400px) 88px;
  gap: 10px;
  margin: 17px 0 24px;

  input,
  button {
    box-sizing: border-box;
    height: 31px;
    border: 0;
    border-radius: 3px;
    font: inherit;
  }

  input {
    min-width: 0;
    padding: 0 15px;
    color: ${({ theme }) => theme.colors.text};
    background-color: #ffffff;
  }

  button {
    color: #222222;
    background-color: #f0e9ed;
    cursor: pointer;
  }

  @media (max-width: 560px) {
    grid-template-columns: 1fr;

    button {
      width: 100px;
    }
  }
`;

export const CountdownStyled = styled.p`
  font-weight: 700;
`;

export const PrimaryLogosStyled = styled.div`
  display: flex;
  align-items: flex-start;
  gap: 46px;
  margin: 0 0 34px 4px;

  @media (max-width: 500px) {
    flex-wrap: wrap;
  }
`;

export const UndpLogoStyled = styled.img`
  width: 83px;
  height: auto;
`;

export const VerdensklasseLogoStyled = styled.img`
  width: min(259px, 60vw);
  height: auto;
`;

export const SecondaryLogosStyled = styled.div`
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  gap: 23px;
  margin-left: 4px;
`;

export const GymnasierLogoStyled = styled.img`
  width: min(366px, 100%);
  height: auto;
`;

export const DanidaLogoStyled = styled.img`
  width: 104px;
  height: auto;
`;
